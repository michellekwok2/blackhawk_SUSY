...
int main() {
	...
	for(int k = 0;k<nb_init_en;k++){
		...
		for (int iEvent = 0; iEvent < nb_events; ++iEvent) {
			...
			for(int i = 0;i<pythia.event.size();i++){
				...
				if(pythia.event[i].isFinal() && (pythia.event[i].id() == 13 || pythia.event[i].id() == -13)){ // muon counter
					if(pythia.event[i].e()>Emin_fin && pythia.event[i].e()<Emax_fin){
						counter = 0;
						while(pythia.event[i].e()>final_energies[counter]){
							counter++;
				...
				if(pythia.event[i].isFinal() && (pythia.event[i].id() == 211 || pythia.event[i].id() == -211)){ // pi+- counter
					if(pythia.event[i].e()>Emin_fin && pythia.event[i].e()<Emax_fin){
						counter = 0;
						while(pythia.event[i].e()>final_energies[counter]){
							counter++;
				...
				if(pythia.event[i].isFinal() && pythia.event[i].id() == 130){ // K0L counter
					if(pythia.event[i].e()>Emin_fin && pythia.event[i].e()<Emax_fin){
						counter = 0;
						while(pythia.event[i].e()>final_energies[counter]){
							counter++;
				...
				if(pythia.event[i].isFinal() && (pythia.event[i].id() == 321 || pythia.event[i].id() == -321)){ // K+- counter
					if(pythia.event[i].e()>Emin_fin && pythia.event[i].e()<Emax_fin){
						counter = 0;
						while(pythia.event[i].e()>final_energies[counter]){
							counter++;
				...
				if(pythia.event[i].isFinal() && (pythia.event[i].id() == 2112 || pythia.event[i].id() == -2112)){ // neutron counter
					if(pythia.event[i].e()>Emin_fin && pythia.event[i].e()<Emax_fin){
						counter = 0;
						while(pythia.event[i].e()>final_energies[counter]){
							counter++;
	...
	ofstream file("table_strange.txt",ios::out);
	...
	for(int i = 0;i<nb_init_en;i++){
		for(int j = 0;j<nb_fin_en;j++){
			...
			file	<< "\t" << setw(WIDTH) << (float)photon[i][j]/nb_events
					...
					<< "\t" << setw(WIDTH) << (float)muon[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)nu_e[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)nu_mu[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)nu_tau[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)pipm[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)K0L[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)Kpm[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)proton[i][j]/nb_events
					<< "\t" << setw(WIDTH) << (float)neutron[i][j]/nb_events
					<< endl;
  		}
	}
  	file.close();
