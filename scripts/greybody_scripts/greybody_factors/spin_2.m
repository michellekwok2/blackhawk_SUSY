...
lambda[l_,m_,s_,gamma_]:=If[gamma==0,coef0[l]-6,coef0[l]-coef1[l,m,s]*gamma+coef2[l,m,s]*gamma^2-coef3[l,m,s]*gamma^3+coef4[l,m,s]*gamma^4-coef5[l,m,s]*gamma^5+coef6[l,m,s]*gamma^6+gamma^2+2*m*gamma-6];
Delta[r_,M_,a_]:=r^2-2*M*r+a^2*M^2;
Deltaprime[r_,M_]:=2*(r-M);
...
invertr[rs_,M_,a_,omega_,m_]:=If[a==0,2*M*(1+ProductLog[Exp[rs/(2*M)-1]]),Module[{start=If[rs<=5,rplus[M,a]+0.00001,rs]},Re[Evaluate[u/.FindRoot[rstar[u,M,a,omega,m]==rs,{u,start}]]]]];
...
(* ::Input::Initialization:: *)
choice1=2;(*choice for beta_2, 1 is upper sign*)
choice2=1;(*choice for kappa_2, 1 is upper sign*)
q[r_,M_,a_,omega_,l_,m_]:=(lambda[l,m,s,omega*M*a]+4)*rho2[r,M,a,omega,m]^2+3*rho2[r,M,a,omega,m]*(r^2-a^2*M^2)-3*r^2*Delta[r,M,a];
qprime[r_,M_,a_,omega_,l_,m_]:=r*((4*(lambda[l,m,s,omega*M*a]+4)+6)*rho2[r,M,a,omega,m]-6*(r^2-3*M*r+2*a^2*M^2));
qsecond[r_,M_,a_,omega_,l_,m_]:=(4*(lambda[l,m,s,omega*M*a]+4)+6)*rho2[r,M,a,omega,m]+8*(lambda[l,m,s,omega*M*a]+4)*r^2-6*r^2+36*M*r-12*a^2*M^2;
qbeta[r_,M_,a_,omega_,l_,m_]:=If[choice1==1,rho2[r,M,a,omega,m]*((lambda[l,m,s,omega*M*a]+4)*rho2[r,M,a,omega,m]+6*M*r-6*a^2*M^2),(lambda[l,m,s,omega*M*a]+4)*rho2[r,M,a,omega,m]^2+6*r^2*a*M*m/omega+6*M*r*(r^2-alpha2[M,a,omega,m])];
qDelta[r_,M_,a_,omega_,l_,m_]:=-2*(r-M)*(lambda[l,m,s,omega*M*a]+4)*rho2[r,M,a,omega,m]^2+2*rho2[r,M,a,omega,m]*(2*(lambda[l,m,s,omega*M*a]+4)*r*Delta[r,M,a]-3*M*r^2-3*M^3*a^2+6*r*a^2*M^2)+12*r*Delta[r,M,a]*(M*r-a^2*M^2);
beta2[M_,a_,omega_,m_]:=If[choice1==1,3*alpha2[M,a,omega,m],-3*alpha2[M,a,omega,m]];
kappa2[M_,a_,omega_,l_,m_]:=If[choice2==1,Sqrt[36*M^2-2*(lambda[l,m,s,omega*M*a]+4)*(alpha2[M,a,omega,m]*(5*(lambda[l,m,s,omega*M*a]+4)+6)-12*a^2*M^2)+2*beta2[M,a,omega,m]*(lambda[l,m,s,omega*M*a]+4)*(lambda[l,m,s,omega*M*a]+6)],-Sqrt[36*M^2-2*(lambda[l,m,s,omega*M*a]+4)*(alpha2[M,a,omega,m]*(5*(lambda[l,m,s,omega*M*a]+4)+6)-12*a^2*M^2)+2*beta2[M,a,omega,m]*(lambda[l,m,s,omega*M*a]+4)*(lambda[l,m,s,omega*M*a]+6)]];
V2[r_,M_,a_,omega_,l_,m_]:=Delta[r,M,a]/rho2[r,M,a,omega,m]^4*(q[r,M,a,omega,l,m]-rho2[r,M,a,omega,m]/qbeta[r,M,a,omega,l,m]^2*(qbeta[r,M,a,omega,l,m]*(rho2[r,M,a,omega,m]*Delta[r,M,a]*qsecond[r,M,a,omega,l,m]-2*rho2[r,M,a,omega,m]*q[r,M,a,omega,l,m]-2*r*qDelta[r,M,a,omega,l,m])+rho2[r,M,a,omega,m]*(kappa2[M,a,omega,l,m]*rho2[r,M,a,omega,m]-qprime[r,M,a,omega,l,m]+beta2[M,a,omega,m]*Deltaprime[r,M])*qDelta[r,M,a,omega,l,m]));
...
coef[M_,a_,omega_,m_]:=If[choice1==2,func1[rstar[rdiv-prox,M,a,omega,m]]/Sqrt[prox],func1[rstar[rdiv-prox,M,a,omega,m]]*Sqrt[prox]];
printsteps=0;
far = 1000.;
testl[M_,a_,omega_,l_,m_]:=Quiet[Check[NDSolve[{Xl''[rs]+omega^2*Xl[rs]==V2[coordl[rs],M,a,omega,l,m]*Xl[rs],{Xl[rminf]== Ainh*Exp[-I*omega*rminf],Xl'[rminf] == -I*omega*Ainh*Exp[-I*omega*rminf]}},Xl,{rs,rstar[rdiv-prox,M,a,omega,m],rminf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]];
testr[M_,a_,omega_,l_,m_]:=If[choice1==2,Quiet[Check[NDSolve[{X''[rs]+omega^2*X[rs]==V2[coordr[rs],M,a,omega,l,m]*X[rs],{X[rstar[rdiv+prox,M,a,omega,m]]==coef[M,a,omega,m]*Sqrt[prox],X'[rstar[rdiv+prox,M,a,omega,m]] == coef[M,a,omega,m]/2*Delta[rdiv+prox,M,a]/rho2[rdiv+prox,M,a,omega,m]/Sqrt[prox]}},X,{rs,rstar[rdiv+prox,M,a,omega,m],rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]],Quiet[Check[NDSolve[{X''[rs]+omega^2*X[rs]==V2[coordr[rs],M,a,omega,l,m]*X[rs],{X[rstar[rdiv+prox,M,a,omega,m]]==coef[M,a,omega,m]/Sqrt[prox],X'[rstar[rdiv+prox,M,a,omega,m]] == -coef[M,a,omega,m]/2*Delta[rdiv+prox,M,a]/rho2[rdiv+prox,M,a,omega,m]/prox^(3/2)}},X,{rs,rstar[rdiv+prox,M,a,omega,m],rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]]];
testing[M_,a_,omega_,l_,m_]:=Module[{temp=testl[M,a,omega,l,m]},soll=temp;testr[M,a,omega,l,m]];
solver[M_,a_,omega_,l_,m_]:=If[omega>-a*m/(2*rplus[M,a]),print=0;prox=10^(-4);rminf=rstar[rplus[M,a]+prox,M,a,omega,m];rpinf=rstar[rplus[M,a]+far,M,a,omega,m];If[printsteps==1,Print[1]];tortoise=NDSolve[{r'[rs]==(r[rs]^2+a^2*M^2-2*M*r[rs])/(r[rs]^2+a^2*M^2+a*M*m/omega),{r[rminf]==rplus[M,a]+prox}},r,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];If[printsteps==1,Print[2]];choice1=2;choice2=1;test=Quiet[Check[NDSolve[{X''[rs]+omega^2*X[rs]==V2[coord[rs],M,a,omega,l,m]*X[rs],{X[rminf]==Ainh*Exp[-I*omega*rminf],X'[rminf] == -I*omega*Ainh*Exp[-I*omega*rminf]}},X,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]];If[test==1,If[printsteps==1,Print[3]];choice1=1;choice2=1;test=Quiet[Check[NDSolve[{X''[rs]+omega^2*X[rs]==V2[coord[rs],M,a,omega,l,m]*X[rs],{X[rminf]==Ainh*Exp[-I*omega*rminf],X'[rminf] == -I*omega*Ainh*Exp[-I*omega*rminf]}},X,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]];If[test==1,If[printsteps==1,Print[4]];choice1=1;choice2=2;test=Quiet[Check[NDSolve[{X''[rs]+omega^2*X[rs]==V2[coord[rs],M,a,omega,l,m]*X[rs],{X[rminf]== Ainh*Exp[-I*omega*rminf],X'[rminf] == -I*omega*Ainh*Exp[-I*omega*rminf]}},X,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]];If[test==1,If[printsteps==1,Print[5]];choice1=2;choice2=2;test=Quiet[Check[NDSolve[{X''[rs]+omega^2*X[rs]==V2[coord[rs],M,a,omega,l,m]*X[rs],{X[rminf]== Ainh*Exp[-I*omega*rminf],X'[rminf] == -I*omega*Ainh*Exp[-I*omega*rminf]}},X,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],1,{NDSolve::ndsz}]]]]];test,print=1;rdiv=Sqrt[-alpha2[M,a,omega,m]];If[omega<0.1,If[a<0.9,prox=10^(-3),prox=10^(-2)],prox=10^(-4)];rminf=rstar[rplus[M,a]+prox,M,a,omega,m];rpinf=rstar[rplus[M,a]+far,M,a,omega,m];If[printsteps==1,Print[1]];tortoisel=NDSolve[{r'[rs]==(r[rs]^2+a^2*M^2-2*M*r[rs])/(r[rs]^2+a^2*M^2+a*M*m/omega),{r[rminf]==rplus[M,a]+prox}},r,{rs,rstar[rdiv-prox,M,a,omega,m],rminf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];If[printsteps==1,Print[2]];tortoiser=NDSolve[{r'[rs]==(r[rs]^2+a^2*M^2-2*M*r[rs])/(r[rs]^2+a^2*M^2+a*M*m/omega),{r[rstar[rdiv+prox,M,a,omega,m]]==rdiv+prox}},r,{rs,rstar[rdiv+prox,M,a,omega,m],rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];If[printsteps==1,Print[3]];choice1=2;choice2=1;tests=testing[M,a,omega,l,m];If[tests==1,choice1=2;choice2=2;If[printsteps==1,Print[4]];tests=testing[M,a,omega,l,m];If[tests==1,choice1=1;choice2=2;If[printsteps==1,Print[5]];tests=testing[M,a,omega,l,m];If[tests==1,choice1=1;choice2=1;If[printsteps==1,Print[6]];tests=testing[M,a,omega,l,m]]]];tests];


(* ::Input::Initialization:: *)
SetDirectory[NotebookDirectory[]];
file1=OpenWrite["test_2_fM.txt"];
file2=OpenWrite["test_2_gM.txt"];
...
optimize=1;
For[k=1,k<nba+1,k++,For[j=1,j<nbener+1,j++,Al0[[k,j]]=0;Al1[[k,j]]=0;For[i=1,i<nbmodes+1,i++,If[optimize==1 && i>1 && Al0[[k,j]]>10^(-100) && Abs[Max[contrib[[k,j,i-1]]]/Al0[[k,j]]]<0.00001,i=nbmodes+1,For[m=-l[[i]],m<l[[i]]+1,m++,If[a[[k]]!=0 && optimize==1 && m>-l[[i]] && Al0[[k,j]]>10^(-100) && Abs[contrib[[k,j,i,m+l[[i]]]]/Al0[[k,j]]]<0.00001,m=l[[i]]+1,If[a[[k]]==0 && m>-l[[i]],s0[[k,j,i,m+l[[i]]+1]]=s0[[k,j,i,m+l[[i]]]],s0[[k,j,i,m+l[[i]]+1]]=solver[M,a[[k]],omega[[j]],l[[i]],m]];Module[{Aout=Switch[s0[[k,j,i,m+l[[i]]+1]],1,Infinity,_,(Evaluate[X/.s0[[k,j,i,m+l[[i]]+1]]][[1]][rpinf]-Evaluate[X/.s0[[k,j,i,m+l[[i]]+1]]][[1]]'[rpinf]/(I*omega[[j]]))/(2*Exp[-I*omega[[j]]*rpinf])]},contrib[[k,j,i,m+l[[i]]+1]]=(-1)^print/Abs[Aout]^2/(Exp[4*Pi*(1+1/Sqrt[1-a[[k]]^2])*omega[[j]]*M+2*Pi*m*a[[k]]/Sqrt[1-a[[k]]^2]]-1)];Al0[[k,j]]=Al0[[k,j]]+contrib[[k,j,i,m+l[[i]]+1]];Al1[[k,j]]=Al1[[k,j]]-m*contrib[[k,j,i,m+l[[i]]+1]];If[print==1,Print[{k,j,l[[i]],m,contrib[[k,j,i,m+l[[i]]+1]]}," -> superradiance"],Print[{k,j,l[[i]],m,contrib[[k,j,i,m+l[[i]]+1]]}]]]]]]];WriteLine[file1,ExportString[{Al0[[k]]},"TSV"]];WriteLine[file2,ExportString[{Al1[[k]]},"TSV"]]];
Close[file1];
Close[file2];
Print["DONE"]
