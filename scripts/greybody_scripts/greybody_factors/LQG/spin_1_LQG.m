...
(* ::Input::Initialization:: *)
V1[r_,M_,epsilon_,a0_,l_]:=nu1[l]*G[r,M,epsilon,a0]/H[r,a0];
...
rpinf = rstar[rplus[M,epsilon]+far,M,epsilon,a0]},tortoise=NDSolve[{r'[rs]==r[rs]^2*(r[rs]-rplus[M,epsilon])*(r[rs]-rminus[M,epsilon])/(r[rs]^4+a0^2),{r[rminf]==rplus[M,epsilon]+prox}},r,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];NDSolve[{X''[rs]+omega^2*X[rs]==V1[coord[rs],M,epsilon,a0,l]*X[rs],{X[rminf]== Ainh*Exp[-I*omega*rminf],X'[rminf] == -I*omega*Ainh*Exp[-I*omega*rminf]}},X,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10]];


(* ::Input::Initialization:: *)
SetDirectory[NotebookDirectory[]];
file1=OpenWrite["fM_LQG_1_low.txt"];
