...
(* ::Input::Initialization:: *)
H[l_,alpha_,beta_,s_]:=If[l==0,0,If[l==0.5 || l==-0.5,(l^2-(alpha+beta)^2/4)*(l^2-(alpha-beta)^2/4)/(2*l^3),(l^2-(alpha+beta)^2/4)*(l^2-s^2)*(l^2-(alpha-beta)^2/4)/(2*(l-1/2)*l^3*(l+1/2))]];
coef0[l_]:=l*(l+1);
...
lambda[l_,m_,s_,gamma_]:=If[gamma==0,coef0[l]-3/4,coef0[l]-coef1[l,m,s]*gamma+coef2[l,m,s]*gamma^2-coef3[l,m,s]*gamma^3+coef4[l,m,s]*gamma^4-coef5[l,m,s]*gamma^5+coef6[l,m,s]*gamma^6+gamma^2+2*m*gamma-3/4];
...
invertleft[rs_,M_,a_,omega_,m_]:=Re[Evaluate[u/.FindRoot[rstar[u,M,a,omega,m]==rs,{u,rplus[M,a]+0.00001}]]];
invertright[rs_,M_,a_,omega_,m_]:=Re[Evaluate[u/.FindRoot[rstar[u,M,a,omega,m]==rs,{u,Sqrt[-alpha2[M,a,omega,m]]*1.01}]]];


(* ::Input::Initialization:: *)
epsilon=-1;
V05[r_,M_,a_,omega_,l_,m_]:=(lambda[l,m,s,omega*M*a]+1)*Delta[r,M,a]/rho2[r,M,a,omega,m]^2-epsilon*Sqrt[(lambda[l,m,s,omega*M*a]+1)*Delta[r,M,a]]/rho2[r,M,a,omega,m]^2*((r-M)-2*r*Delta[r,M,a]/rho2[r,M,a,omega,m]);
...
derivate[x_]:=D[c4*coef*u*BesselK[2,I*Sqrt[coef*u]],u]/.{u->x};
printsteps=0;
far = 1000.;
solver[M_,a_,omega_,l_,m_]:=If[omega>-a*m/(2*rplus[M,a]),print=0;prox=10^(-5);rminf=rstar[rplus[M,a]+prox,M,a,omega,m];rpinf=rstar[rplus[M,a]+far,M,a,omega,m];If[printsteps==1,Print[1]];tortoise=NDSolve[{r'[rs]==(r[rs]^2+a^2*M^2-2*M*r[rs])/(r[rs]^2+a^2*M^2+a*M*m/omega),{r[rminf]==rplus[M,a]+prox}},r,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];If[printsteps==1,Print[2]];NDSolve[{X''[rs]+omega^2*X[rs]==V05[coord[rs],M,a,omega,l,m]*X[rs],{X[rminf]== Ainh*Exp[-I*omega*rminf],X'[rminf] ==-I*omega*Ainh*Exp[-I*omega*rminf]}},X,{rs,rminf,rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10],print=1;rdiv=Sqrt[-alpha2[M,a,omega,m]];If[omega<0.1,If[a>=0.9,prox=Min[10^(-2),(rdiv-rplus[M,a])/100.],prox=Min[10^(-3),(rdiv-rplus[M,a])/100.]],prox=Min[10^(-4),(rdiv-rplus[M,a])/100.]];rminf=rstar[rplus[M,a]+prox,M,a,omega,m];rpinf=rstar[rplus[M,a]+far,M,a,omega,m];If[printsteps==1,Print[1]];tortoisel=NDSolve[{r'[rs]==(r[rs]^2+a^2*M^2-2*M*r[rs])/(r[rs]^2+a^2*M^2+a*M*m/omega),{r[rminf]==rplus[M,a]+prox}},r,{rs,rstar[rdiv-prox,M,a,omega,m],rminf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];If[printsteps==1,Print[2]];If[omega<0.1,If[a>=0.9,prox2=Min[10^(-2),(rdiv-rplus[M,a])/100.],prox2=Min[10^(-3),(rdiv-rplus[M,a])/100.]],prox2=Min[10^(-4),(rdiv-rplus[M,a])/100.]];tortoiser=NDSolve[{r'[rs]==(r[rs]^2+a^2*M^2-2*M*r[rs])/(r[rs]^2+a^2*M^2+a*M*m/omega),{r[rstar[rdiv+prox2,M,a,omega,m]]==rdiv+prox2}},r,{rs,rstar[rdiv+prox2,M,a,omega,m],rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->18];If[printsteps==1,Print[3]];soll=NDSolve[{Xl''[rs]+omega^2*Xl[rs]==V05[coordl[rs],M,a,omega,l,m]*Xl[rs],{Xl[rminf]==Ainh*Exp[I*omega*rminf],Xl'[rminf]==I*omega*Ainh*Exp[I*omega*rminf]}},Xl,{rs,rstar[rdiv-prox,M,a,omega,m],rminf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10];coef=4*Sqrt[(lambda[l,m,s,omega*M*a]+1)/Delta[rdiv,M,a]];c4=func1[rstar[rdiv-prox,M,a,omega,m]]/(coef*prox*BesselK[2,Sqrt[coef*prox]]);If[printsteps==1,Print[4]];NDSolve[{X''[rs]+omega^2*X[rs]==V05[coordr[rs],M,a,omega,l,m]*X[rs],{X[rstar[rdiv+prox2,M,a,omega,m]]==c4*coef*prox2*BesselK[2,I*Sqrt[coef*prox2]],X'[rstar[rdiv+prox2,M,a,omega,m]] == Delta[rdiv+prox2,M,a]/rho2[rdiv+prox2,M,a,omega,m]*derivate[prox2]}},X,{rs,rstar[rdiv+prox2,M,a,omega,m],rpinf},Method->"StiffnessSwitching",AccuracyGoal->18,PrecisionGoal->10]];


(* ::Input::Initialization:: *)
SetDirectory[NotebookDirectory[]];
file1=OpenWrite["test_0.5_fM.txt"];
file2=OpenWrite["test_0.5_gM.txt"];
...
optimize=1;
For[k=1,k<nba+1,k++,For[j=1,j<nbener+1,j++,Al0[[k,j]]=0;Al1[[k,j]]=0;For[i=1,i<nbmodes+1,i++,If[optimize==1 && i>1 && Al0[[k,j]]>10^(-100) && Abs[Max[contrib[[k,j,i-1]]]/Al0[[k,j]]]<0.00001,i=nbmodes+1,For[m=-l[[i]],m<l[[i]]+1,m++,If[a[[k]]!=0 && optimize==1 && m>-l[[i]] && Al0[[k,j]]>10^(-100) && Abs[contrib[[k,j,i,m+l[[i]]]]/Al0[[k,j]]]<0.00001,m=l[[i]]+1,If[a[[k]]==0 && m>-l[[i]],s0[[k,j,i,m+l[[i]]+1]]=s0[[k,j,i,m+l[[i]]]],s0[[k,j,i,m+l[[i]]+1]]=solver[M,a[[k]],omega[[j]],l[[i]],m]];Module[{rpinf=rstar[rplus[M,a[[k]]]+far,M,a[[k]],omega[[j]],m]},Ain0[[k,j,i,m+l[[i]]+1]]=(Evaluate[X/.s0[[k,j,i,m+l[[i]]+1]]][[1]][rpinf]+Evaluate[X/.s0[[k,j,i,m+l[[i]]+1]]][[1]]'[rpinf]/(I*omega[[j]]))/(2*Exp[I*omega[[j]]*rpinf]);Aout0[[k,j,i,m+l[[i]]+1]]=(Evaluate[X/.s0[[k,j,i,m+l[[i]]+1]]][[1]][rpinf]-Evaluate[X/.s0[[k,j,i,m+l[[i]]+1]]][[1]]'[rpinf]/(I*omega[[j]]))/(2*Exp[-I*omega[[j]]*rpinf])];contrib[[k,j,i,m+l[[i]]+1]]=1/Abs[Aout0[[k,j,i,m+l[[i]]+1]]]^2/(Exp[4*Pi*(1+1/Sqrt[1-a[[k]]^2])*omega[[j]]*M+2*Pi*m*a[[k]]/Sqrt[1-a[[k]]^2]]+1);Al0[[k,j]]=Al0[[k,j]]+contrib[[k,j,i,m+l[[i]]+1]];Al1[[k,j]]=Al1[[k,j]]-m*contrib[[k,j,i,m+l[[i]]+1]];If[print==1,Print[{k,j,l[[i]],m,contrib[[k,j,i,m+l[[i]]+1]]}," -> superradiance"],Print[{k,j,l[[i]],m,contrib[[k,j,i,m+l[[i]]+1]]}]]]]]]];WriteLine[file1,ExportString[{Al0[[k]]},"TSV"]];WriteLine[file2,ExportString[{Al1[[k]]},"TSV"]]];
Close[file1];
Close[file2];
Print["DONE"]
