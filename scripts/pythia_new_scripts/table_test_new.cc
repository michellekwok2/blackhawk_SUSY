...
const int nb_events = 10;
const int nb_fin_en = 100;
const int nb_init_en = 10;
const double Emin_init = 1.;
const double Emax_init = 10.;
...
int main() {
	char *beam_energy1 = new char [64];
	char *beam_energy2 = new char [64];
	Pythia pythia;
	...
	pythia.readString("23:onIfAny = 11"); // decay in e+-
	pythia.readString("22:onMode = off");
	pythia.readString("22:onIfAny = 11"); // decay in e+-
	...
	for(int k = 0;k<nb_init_en;k++){
		cout << k+1 << "/" << nb_init_en << "\t";
		sprintf(beam_energy1,"Beams:eA = %lf",beam_energies[k]);
		sprintf(beam_energy2,"Beams:eB = %lf",beam_energies[k]);
		printf("\n%s\n%s",beam_energy1,beam_energy2);
		fflush(stdout);
		pythia.readString(beam_energy1);
		pythia.readString(beam_energy2);
		pythia.init();
	...
	ofstream file("table_test.txt",ios::out);
